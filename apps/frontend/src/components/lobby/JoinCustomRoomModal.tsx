...
interface JoinCustomRoomModalProps {
  ...
  onCreate: (username: string, roomName: string, avatarId: string) => void;
}
...
export function JoinCustomRoomModal({ onBack, onJoin, onCreate }: JoinCustomRoomModalProps) {
  ...
  const handleCreate = () => {
    ...
    setError(""); setLoading(true);
    // Custom logic to handle creation through prop
    onCreate(username.trim(), roomName.trim(), AVATARS[avatarIndex].id);
  };
