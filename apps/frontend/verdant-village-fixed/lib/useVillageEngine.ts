  ...
  const interact = useCallback(() => {
    ...
    if (obj.type === "door") {
      setOpenDoors((prev) => {
        ...
        if (next.has(obj.id)) {
          next.delete(obj.id);
          showNotif("🚪 Door closed.");
        } else {
          next.add(obj.id);
          showNotif(`🚪 ${obj.label} — Enter!`);
        }
    ...
    const loop = () => {
      ...
      setPlayer((prev) => {
        ...
        if (dx !== 0 || dy !== 0) {
          ...
          const collision = buildCollisionMap(openDoorsRef.current);
          const nx = x + vx;
          const ny = y + vy;
      ...
      setPlayer((prev) => {
        ...
        for (const obj of OBJECTS) {
          ...
          if (d < 2.2 && d < closestObjDist) { closestObj = obj.id; closestObjDist = d; }
        }
