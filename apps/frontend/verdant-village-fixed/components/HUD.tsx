...
export default function HUD({
  ...
  dialogueOpen, setDialogueOpen, activeDialogue, onInteract, openDoors,
}: HUDProps) {
  ...
  return (
    <>
      ...
      {(nearbyObj || nearbyNPC) && !dialogueOpen && (() => {
        const obj = nearbyObj ? OBJECTS.find(o => o.id === nearbyObj) : null;
        const isDoor = obj?.type === "door";
        const isOpen = isDoor && openDoors.has(obj!.id);
        const label = nearbyNPC
          ? `Talk to ${nearbyNPC.name}`
          : isDoor
          ? (isOpen ? `Close ${obj!.label}` : `Enter ${obj!.label}`)
          : obj?.type === "chest"
          ? "Open chest"
          : obj?.type === "sign"
          ? "Read sign"
          ...
          ? "Toggle campfire"
          : "Interact";
        const icon = isDoor ? (isOpen ? "🚪" : "🏠") : obj?.type === "chest" ? "📦" : obj?.type === "sign" ? "📜" : "✨";
        return (
          <div
            className="absolute bottom-[185px] left-1/2 -translate-x-1/2 z-30 bg-[#1a2820]/95 border-2 border-[#68c055] px-5 py-2 flex items-center gap-3 shadow-[3px_3px_0_#4a8a3c] cursor-pointer animate-bounce"
